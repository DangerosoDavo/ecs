...
- **Access Declarations**: Systems declare read/write sets; the scheduler enforces per-system uniqueness, blocks async groups from mutating components unless explicitly allowed, and guarantees a single work group owns each writable component. Within a synchronized work group, multiple systems may write the same component or resource because they execute sequentially and share the deferred command buffer.
